...
def recommended_posts_nn_cat(
    ...
    final_data = pd.concat((data['posts'], key_user), axis=1)
    
    #соединяю на основе post_id c custom features, resulted from NNet + drop post_id
    final_data = pd.merge(final_data, 
                          data['post_features'],
                          on='post_id',
                          how='left').drop(['post_id'], axis=1)
